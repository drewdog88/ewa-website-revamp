...
- **The public site loads** at https://www.eastlakewolfpack.org. Clubs, News, Resources, Officers, Fundraiser, and About render their content — or render empty if nothing is published/active. `#privacy` and `#accessibility` open the short notices.
- **An admin can log in** with username, password, and the Cloudflare Turnstile check; the Admin Panel opens and all content editors load.
...
| **Can't log in to admin** (submitted credentials rejected) | 1. Wrong username or password, OR<br>2. Turnstile failed (`403 Security check failed`), OR<br>3. `JWT_SECRET` missing/changed, OR<br>4. Cookie not being set/read (HTTPS required for `Secure` cookies) | Vercel function logs for `/api/auth/login`; Vercel project → Settings → Environment Variables | 1. Verify credentials against `users` (see [Database](Database)).<br>2. Complete the Turnstile widget; if `TURNSTILE_SECRET_KEY` is unset, login skips the check and logs a warning. BotID is advisory unless `BOTID_ENFORCE_LOGIN=1`.<br>3. Check `JWT_SECRET` (see [Configuration](Configuration)).<br>4. Confirm HTTPS. See [How It Works](How-It-Works)#authentication-in-detail. Clear cookies and retry. |
...
| **Site not being indexed by Google** | Search Console delay, or a stale `noindex` cache — the live file is `index, follow` | `index.html`, `public/robots.txt` | Confirm the deployed HTML has `index, follow`. AI crawlers are blocked on purpose (`robots.txt` + `middleware.ts`). See [Deployment](Deployment). |
| **`ops-log write failed: fetch failed` in Vercel logs** | Best-effort write to private R2 `stats.json` missed (network or contention) | Vercel function logs; Grafana if you run the NAS stack | **Not a site outage.** Login and content still succeeded. See [Observability](Observability). |
