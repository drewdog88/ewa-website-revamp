...
const BLACK = "#111111";
const NAVY = "#2c3e50";
const IRS_RED = "#dc3545";
// Footer stays a dark frame; hero/nav are light in this pass.
    ...
    <div className="min-h-screen bg-background text-foreground" style={{ fontFamily: "var(--font-body)" }}>
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "shadow-md" : "border-b border-border"}`}
        ...
        <nav className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a key={link.label} href={link.href}
                className="transition-colors text-xs font-bold tracking-[0.15em] uppercase hover:opacity-70"
                style={{ color: NAVY }}>
                {link.label}
              </a>
            ))}
          ...
          <button className="md:hidden p-2" style={{ color: NAVY }} onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu">
            {menuOpen ? <X size={22} /> : <Menu size={22} />}
          </button>
        </nav>

        {menuOpen && (
          <div className="md:hidden border-t border-border px-6 py-4 flex flex-col gap-1 bg-white">
            {navLinks.map((link) => (
              <a key={link.label} href={link.href} onClick={() => setMenuOpen(false)}
                className="text-sm font-bold tracking-widest uppercase py-3 border-b border-border hover:opacity-70"
                style={{ color: NAVY }}>
                {link.label}
              </a>
            ))}
      ...
      <section id="home" className="relative min-h-[80vh] flex items-center overflow-hidden bg-background">
        <div className="relative max-w-7xl mx-auto px-6 pt-28 pb-16 w-full grid md:grid-cols-2 gap-12 items-center">
          <div>
            <div className="inline-flex items-center gap-2 border text-xs font-black tracking-[0.18em] uppercase px-3 py-2 rounded mb-8"
              style={{ borderColor: IRS_RED, background: `${IRS_RED}12`, color: IRS_RED }}>
              501(c)(3) Nonprofit · Tax ID 77-0616862
            </div>
            <h1 className="font-black leading-none uppercase mb-6"
              style={{ fontFamily: "var(--font-display)", color: NAVY, fontSize: "clamp(3rem, 8vw, 5.5rem)", letterSpacing: "-0.01em" }}>
              Eastlake<br />
              <span style={{ color: IRS_RED }}>Wolfpack</span><br />
              Association
            </h1>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <a href="#news" className="border font-bold text-xs px-7 py-4 rounded uppercase tracking-widest hover:bg-white transition-colors"
                style={{ borderColor: "rgba(44,62,80,0.25)", color: NAVY }}>
                Latest News
              </a>
            </div>
          </div>
          ...
          <div className="hidden md:flex justify-center items-center">
            <img src={LOGO_WOLF} alt="Eastlake Wolves" className="w-72 h-72 object-contain drop-shadow-xl" />
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-background border-t border-border">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <div id="about" className="scroll-mt-24">
            ...
            <h2 className="font-black leading-none uppercase mb-8" style={{ fontFamily: "var(--font-display)", color: NAVY, fontSize: "clamp(2.25rem, 4vw, 3.25rem)" }}>
              About EWA
            </h2>
            <div className="space-y-6 text-lg leading-relaxed text-muted-foreground">
              ...
              <p>
                We are a nonprofit organization recognized by the IRS as a{" "}
                <span className="font-black" style={{ color: IRS_RED }}>501(c)(3)</span>
                {" "}with Tax Identification{" "}
                <span className="font-black" style={{ color: IRS_RED }}>#&nbsp;77-0616862</span>.
              </p>
            </div>

            {showFundraiser && (
              <div className="rounded-2xl border border-border p-6 mt-10" style={{ background: `${MAROON}08` }}>
                ...
                <h3 className="font-black uppercase leading-none mb-6"
                  style={{ fontFamily: "var(--font-display)", color: NAVY, fontSize: "clamp(1.5rem, 3vw, 2.25rem)" }}>
                ...
                <div className="flex items-end justify-between mb-2">
                  ...
                  <div className="text-right">
                    <div className="font-black text-xl" style={{ color: NAVY, fontFamily: "var(--font-display)" }}>{dollars(fundraiser!.goalCents)}</div>
          ...
          <div id="clubs" className="scroll-mt-24">
            <div className="mb-8">
              ...
              <h2 className="font-black leading-none uppercase" style={{ fontFamily: "var(--font-display)", color: NAVY, fontSize: "clamp(2rem, 3.5vw, 2.75rem)" }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {activeClubs.map((club) => {
                ...
                return (
                  <div key={club.id} className="bg-card border border-border rounded-2xl p-4 flex flex-col hover:shadow-lg hover:-translate-y-1 transition-all group">
                    <div className="w-8 h-1 rounded-full mb-3 transition-all group-hover:w-12" style={{ background: MAROON }} />
                    <h3 className="font-black uppercase text-base leading-tight mb-2" style={{ fontFamily: "var(--font-display)", color: NAVY }}>
                      {club.name}
                    </h3>
                    <p className="text-muted-foreground text-xs leading-relaxed flex-1 mb-4">{club.description}</p>
                    <div className="flex flex-wrap gap-2 mt-auto">
                      {canPay && (
                        <button
                          onClick={() => setPayingClub(club)}
                          className="flex items-center gap-1.5 text-white font-black text-[10px] px-3 py-2 rounded-lg uppercase tracking-widest transition-colors hover:opacity-90"
                          style={{ background: MAROON }}>
                          <QrCode size={11} /> Pay / Donate
                        </button>
                      )}
                      {club.websiteUrl && club.websiteUrl !== "#" && (
                        <a href={club.websiteUrl} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-1.5 border font-bold text-[10px] px-3 py-2 rounded-lg uppercase tracking-widest transition-colors hover:bg-gray-50"
                          style={{ borderColor: "rgba(0,0,0,0.15)", color: NAVY }}>
